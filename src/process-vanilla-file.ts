...
import { hash, type IdentifierOption } from '@vanilla-extract/integration'
...
interface ProcessVanillaFileOptions {
  ...
  virtualRequire?: (moduleId: string) => string
...
/**
 * Taken from {@link https://github.com/vanilla-extract-css/vanilla-extract/blob/master/packages/integration/src/processVanillaFile.ts | processVanillaFile.ts}
 * to make this function synchronous by making the option `serializeVirtualCssPath` sync and required.
 */
export function processVanillaFile({
...
}: ProcessVanillaFileOptions): string {
  ...
  const evalResult = evalCode(
    adapterBoundSource,
    filePath,
    { console, process, __adapter__: cssAdapter, require: virtualRequire },
    true,
  ...
  for (const [serialisedFileScope, fileScopeCss] of cssByFileScope) {
    ...
    if (serializeVirtualCssPath) {
      let virtualCssFilePath = serializeVirtualCssPath({
        fileName,
        fileScope,
        source: css,
      })

      if (virtualCssFilePath) {
        cssImports.push(virtualCssFilePath)
      }
    } else {
      cssImports.push(`import './${fileName}'`)
    }
  }
  ...
  `,
    filePath,
    { console, process, require: virtualRequire },
    true,
  )
